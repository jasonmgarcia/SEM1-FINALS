        ...
        try {
            ...
            ctx.find("div.ui-select select").each(function () {
                var select = $(this),
                    ...
                    selectedOptions = realOptions.filter("[selected='selected']");
                //We have to exclude template option from select to prevent its showing on device in case of data-native-menu=true
                if (select.attr("data-native-menu") !== undefined &&
                    select.attr("data-native-menu") === "true"){

                    select.off({
                            focus: excludeTplOption,
                            blur: includeTplOption})
                        .on({
                            focus: excludeTplOption,
                            blur: includeTplOption
                        });
                ...
                function includeTplOption() {
                    var tplOption = $(this).parent().find("[data-appery-tpl]").attr("selected", false);
                    $(this).append(tplOption);
                }

                function excludeTplOption() {
                    var tplOption = $(this).find("[data-appery-tpl]");
                    $(this).parent().append(tplOption);
                }
                ...
                if (realOptions.length > 0) {
                    ...
                    if (selectedOptions.length !== 1) {
                        ...
                        if (select.attr("data-deferred-value")) {
                            select.find("option[value='" + select.attr("data-deferred-value") + "']").attr("selected", "selected");
                            select.removeAttr("data-deferred-value");
                        } else {
